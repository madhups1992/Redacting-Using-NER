...
def concepts(text,concept):
    SentenceList = nltk.tokenize.line_tokenize(text)
    list5=[]
    ...
    for line in range(len(SentenceList)):
        words = nltk.tokenize.word_tokenize(SentenceList[line])
        flag = 0
        for j in range(len(words)): 
            for i in range(len(meanings)):
                if (meanings[i] == words[j] and flag == 0):
                    flag = flag + 1
...
def store_redactedfiles(redacted_file,Directory,filename):
    file_pattern = ".redacted"
    file_redc_name=[]
    j=0
    for i in range(len(redacted_file)):
        file_redc_name.append(Directory +'/'+ re.sub('.txt','.redacted',filename[i]))
